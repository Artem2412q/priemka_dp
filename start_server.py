...
def show_all_summary_sku_rows(sheet_root):
    """Show all 12 SKU rows in the upper summary table without touching checklist layout."""
    sheet_data = sheet_root.find(qn("sheetData"))
    ...
    for row in sheet_data.findall(qn("row")):
        ...
        if 5 <= row_num <= 17:
            ...
            row.attrib.pop("outlineLevel", None)
            row.attrib.pop("collapsed", None)
...
def build_excel(state, export_type="new"):
    ...
    with zipfile.ZipFile(template_path, "r") as source:
        ...
        # В исходном V3 строки позиций 6–12 скрыты outline-группировкой.
        # Экспорт всегда раскрывает верхнюю товарную таблицу до 12 SKU.
        show_all_summary_sku_rows(sheet_root)
